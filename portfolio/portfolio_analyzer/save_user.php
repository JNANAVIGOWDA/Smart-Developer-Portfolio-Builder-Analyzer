...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES['resume']) && $_FILES['resume']['error'] == UPLOAD_ERR_OK) {
        ...
        if (in_array($ext, $allowed)) {
            if (move_uploaded_file($file_tmp, $file_dest)) { $resume_path = $file_dest; }
        }
    }
    ...
    if (isset($_FILES['profile_image']) && $_FILES['profile_image']['error'] == UPLOAD_ERR_OK) {
        ...
        $ext = strtolower(pathinfo($file_name, PATHINFO_EXTENSION));
        if (in_array($ext, $allowed)) {
            if (move_uploaded_file($file_tmp, $file_dest)) { $image_path = $file_dest; }
        }
    }

    // Insert User (Note: email must be unique)
    $stmt = $conn->prepare("INSERT INTO users (name, email, github, resume_path, profile_image, tagline, bio, phone, location, career_objective, linkedin) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("sssssssssss", $name, $email, $github, $resume_path, $image_path, $tagline, $bio, $phone, $location, $career_objective, $linkedin);
    ...
    if ($stmt->execute()) {
        ...
        // If email already exists, catch it
        if ($conn->errno == 1062) {
            echo "<script>alert('Email is already registered. Please log in.'); window.location.href='login.html';</script>";
        } else {
